...
def add_dimensions_columns(df):
    ...
    try:
        if 'absolute_path' not in df.columns:
            raise ValueError("DataFrame должен содержать колонку 'absolute_path'")
        ...
        for abs_path in df['absolute_path']:
            try:
                ...
                print(f"Ошибка при обработке пути {abs_path}: {e}")
                dimensions.append((None, None, None))
                failed += 1
        ...
        print(f"Ошибка при добавлении колонок с размерами изображений: {e}")
        return df
